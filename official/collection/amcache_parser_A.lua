--[=[ 
name: Amcache Parsers Forever Me You FBS V2
...
differential = hunt.arg.boolean("differential", false, true) -- Will saved last scan locally and only add new items on subsequent scans.
...
if download then
    ...
    if not hunt.fs.path_exists(binpath) then
...
sep = '|'
oldhashlist = {}
if differential and hunt.fs.path_exists(outpath) then
...
-- Read csv into array
if hunt.fs.path_exists(outpath) then
